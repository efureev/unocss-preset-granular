...
| `includeExtraRules`                     | If `false`, skip the `presetMini` gap‑fill utilities (`animate-*`, `space-*`, `divide-*`, `backdrop-*`, `object-cover`, `object-center`, `sr-only`, `uppercase` & friends) from `@feugene/unocss-mini-extra-rules`. Default: `true` — components reach for these utilities, and without them the class stays in the markup while no CSS is emitted. |
