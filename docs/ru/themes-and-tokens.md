...
Увидеть эту цепочку разрешённой для конкретной сборки — кто написал каждый слой
и чьё значение победило — можно командой
`npx granular tokens <файл-опций> <Компонент>`; см. [CLI `granular`](./cli.md).
...
Пресет селективен по компонентам, но **фундамент — нет**: `tokensCssUrl` и
файлы тем инлайнятся целиком, сколько бы компонентов ни было выбрано. На
реальной дизайн-системе это значит, что приложение, взявшее одну карточку,
получает 212 объявленных CSS-переменных, а нужны ему 17.

`pruneTokens` убирает объявления, до которых ничто не дотягивается.
...
presetGranularNode({
  ...
  pruneTokens: {
    mode: 'report', // 'off' (по умолчанию) | 'report' | 'on'
    appSources: { dirs: ['./src'] },
    keep: ['brand-*'],
  },
})
...
Токен уцелеет, если верно хоть одно из перечисленного, плюс транзитивное
замыкание по значениям (сохранённый токен держит всё, на что ссылается сам):

- его потребляет выбранный компонент — через `safelist`, объявленные
  `cssFiles`, свои исходники или имя строкой в JS;
- на него ссылаются правила инлайнимого CSS (`base.css` — такой файл);
- в него целится `themes.tokenOverrides`;
- его объявляет структурный слой темы;
- он найден в `pruneTokens.appSources`;
- он подошёл под `keep` / `keepPrefixes` или `dynamicTokens` выбранного компонента.

Множество сохранённых **глобально по темам**: токен, использованный только
через значение в тёмной теме, уцелеет и в светлой.

### Что не обрезается никогда

`base.css` — там правила сброса, а не объявления, и неиспользуемое правило
статически не отличить от нужного. Компонентный CSS — он уже селективен по
построению. Структурные блоки тем — их и так приносят только выбранные
компоненты.

### Внедрять через `report`

Пресет видит компоненты провайдера и **не видит** вашу разметку. Написанный в
`App.vue` `bg-[var(--brand)]` для него не существует, и без `appSources` этот
токен уедет из CSS при зелёной сборке.

```bash
granular prune ./granular.options.mjs
```

Прочитайте список удаляемого, допишите недостающее в `appSources` или `keep`,
и только потом переводите `mode` в `'on'`. Разбор отчёта — в [CLI](./cli.md).
...
| Форма | Цена | Когда |
|---|---|---|
| `new URL('./themes/light.css', import.meta.url).href` | CSS инлайнится в JS‑чанк | **Не нужен шаг сборки.** Бандлер распознаёт этот литерал и утаскивает содержимое файла в свой выход — CSS гарантированно есть в опубликованном пакете. |
| `'./themes/light.css'` (строка) | нужен `granularCssAssetsPlugin` | **Меньше бандл.** Ничего не инлайнится, файл кладётся в `dist` по своему `assetName`. |

Разница не косметическая. Бандлер реагирует именно на **литерал**
`new URL(..., import.meta.url)` — и на практике он не кладёт файл рядом с чанком,
а инлайнит его целиком строкой `data:text/css;base64`. На стороне node это
незаметно и бесплатно, но `config.ts` компонента общий для browser‑ и node‑entry,
поэтому base64 уезжает и в **клиентский бандл потребителя**. У пакета с десятком
тематизированных компонентов это уже заметная величина.

Строка для бандлера — просто данные, поэтому сама по себе она файл в `dist` не
принесёт и в опубликованном пакете ссылка повиснет. Полноценной форму делают
fallback по `assetName` (`components/<Name>/<file>` от `packageBaseUrl` — ровно
как `cssFiles` → `cssFileAssetNames`) плюс шаг сборки, который кладёт туда файл, —
см. [`granularCssAssetsPlugin`](./authoring-providers.md#размещение-задекларированного-css-в-dist).
...
Значения — **эффективные**, со всеми применёнными слоями, включая
`themes.tokenOverrides`. Их считает та же функция, из которой эмитится CSS,
поэтому манифест не может показать значение, которого в стилях нет.
