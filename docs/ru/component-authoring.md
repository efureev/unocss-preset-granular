...
| `dependencies` | Только компоненты, которые собранный код **реально импортирует**.         |
...
### Критично про `dependencies`

Критерий один: **собранный код компонента импортирует директорию другого
granular‑компонента**. Не «шаблон» — импорт из `<script setup>`, композабла,
render‑функции или динамический `import()` считается ровно так же.

- ✅ Мой компонент рендерит чужой (`<GrButton>` внутри `GrSidebar.vue`).
- ✅ Чужой компонент подтягивается условно или лениво — `import()` тоже ребро.
- ❌ Я импортирую из чужой директории константу, тип или утилиту, которая
  ничего не рендерит: это не зависимость, а объявив её, вы притащите всем
  своим потребителям весь CSS и safelist донора.
- ❌ Компонент используется рядом в демо или доке, но не внутри моего шаблона.

Что будет, если ребро забыть: пресет сканирует `components/<Name>/` и
подмешивает `safelist` только компонентам из селекции. Приложение с
`components: ['GrSidebar']` не заглянет в `components/GrButton/` — кнопка
внутри сайдбара приедет без фона и без фокус‑кольца. При этом не падает
ничего: провайдер собирается, типы целы, приложение собирается. Проверить —
`granular doctor` с `components: 'all'`: расхождение объявленного графа с
фактическими импортами в `dist` он показывает как `undeclared-dependency`
(→ [cli.md](./cli.md)).
...
### 5.1. Корневой `src/index.ts` пакета (опционально)
...
Этот barrel **необязателен** — он лишь включает удобный корневой импорт
`import { MyButton } from '@your-scope/your-package'`. Пресету он **не** нужен:
компоненты обнаруживаются через `provider.components` (§5.2) и подключаются по
subpath‑экспорту `./components/<Name>`. Можно экспортировать здесь только часть
компонентов (эталонный `@feugene/simple-package` так и делает). Обязательные
точки подключения — §5.2 и §5.3.
...
4. **Final override в приложении.** Приложение поверх компонента фиксирует
   бренд. Плоская форма (пишет в первичный селектор темы):
   `tokenOverrides: { light: { brand: '#0070f3' } }`; либо вложенная форма для
   конкретного селектора:
   `tokenOverrides: { light: { ':root': { brand: '#0070f3' } } }`. Токены — без
   префикса `--`; в любом случае победа за приложением.
