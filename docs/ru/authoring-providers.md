...
где были. Ключ компонента — это ровно `./components/<Name>`: один
сегмент и без подстановки. Более глубокий subpath того же компонента
(`./components/GrAlert/styles.css`) и паттерн (`./components/*/styles.css`)
принадлежат пакету, а не генератору — ряд их не трогает, как не трогает `.` и
`./contract`. После первой же перезаписи такие ключи встают за рядом и там
остаются.

### Части составных компонентов

`GrTimelineItem`, `GrListItem`, пункты меню лежат в каталоге родителя и
публичными компонентами не считаются: своих `index.ts` и `config.ts` у них нет,
а код и так уезжает в чанк родителя. Отдельная entry им не нужна — а вот subpath
нужен: без него `@feugene/kit/components/GrTimelineItem` упирается в
`ERR_PACKAGE_PATH_NOT_EXPORTED`, то есть гранулярный импорт на эти имена не
распространяется, хотя в шаблоне они пишутся такими же, как остальные.

`packageExports({ subcomponents: true })` ставит рядом с компонентами алиасы:
ключ свой, модуль — родительский. Часть узнаётся по баррелю родителя
(`export { default as GrX } from './GrX.vue'`), поэтому реэкспорт под чужим
именем алиасом не станет — subpath вёл бы к модулю, где такого файла нет.

Часть может лежать в подкаталоге родителя (`parts/GrTimelineItem.vue`), кавычки
любые — модуль всё равно родительский. Имя обязано нести префикс пакета: баррель
реэкспортирует и служебное (`TableCell`), а публичный API пакета генератор сам
не расширяет. Занятое имя — компонентом или частью другого родителя — это ошибка
`subcomponent-name-clash`, а не молчаливый выбор победителя: один subpath не
обслужит два модуля.

Алиас ведёт в модуль родителя, у которого `export default` — сам родитель,
поэтому часть импортируется по имени: `import { GrTimelineItem } from
'@feugene/kit/components/GrTimelineItem'`.

По умолчанию опция выключена: `package.json` провайдера не должен меняться от
одного лишь обновления пресета.
...
Пакет, в котором компонентов ещё нет, — законный первый прогон: вставлять
нечего и якорить не к чему, поэтому `exports` остаётся нетронутым. Чего
генератор делать не станет — считать ОТСУТСТВУЮЩУЮ директорию компонентов
нулём компонентов: опечатка в пути тогда молча вычистила бы каждый реестр.

## Токены, адресуемые в рантайме — `dynamicTokens`

Если компонент собирает имя токена в рантайме, статический анализ его
не найдёт:

```ts
const zVar = '--gr-z-dropdown'
const zIndex = `var(${zVar})` // `var(--gr-z-dropdown)` не встречается нигде
```

Скан имён классов такие токены переживают спокойно, но приложение, включившее
`pruneTokens`, удалит их объявления — и поломка будет тихой: `z-index`
разрешится в `unset`, и панель уедет под соседнюю.

Объявляйте их **на компоненте, который их читает**:

```ts
export const grPopoverConfig = defineGranularComponent(import.meta.url, {
  name: 'GrPopover',
  // Имена без `--`; допускается `*` в конце.
  dynamicTokens: ['gr-z-dropdown'],
})
```

На компоненте, а не на провайдере, по трём причинам. Провайдерский список
держал бы токен в **каждом** приложении, включая те, где этого компонента нет,
— ровно тот перерасход, ради устранения которого существует гранулярный отбор.
Знание живёт рядом с кодом, который его порождает: убрали рантайм-сборку —
видно, что и строка протухла. И потребитель не может знать, что внутри
выбранного им компонента имя собирается в рантайме.

Поле про **потребление**, а не про владение: компонент вправе объявить токен,
который читает у соседа. Оно опционально и без обрезки на эмиссию не влияет.

`granular prune` предупреждает, когда имя удаляемого токена встречается
литералом в файле, который тут же собирает `var()` — это недостающее
объявление подаёт голос вместо того, чтобы CSS сломался молча.
