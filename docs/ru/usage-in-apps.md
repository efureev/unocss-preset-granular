...
| `includeExtraRules`                     | `false` — не добирать утилиты, которых нет в `presetMini` (`animate-*`, `space-*`, `divide-*`, `backdrop-*`), из `@feugene/unocss-mini-extra-rules`. По умолчанию `true`: компоненты этими утилитами пользуются, а без правил класс остаётся в разметке, и CSS к нему не появляется. |
