...
| `includeExtraRules`                     | `false` — не добирать утилиты, которых нет в `presetMini` (`animate-*`, `space-*`, `divide-*`, `backdrop-*`, `object-cover`, `object-center`, `tabular-nums`, `sr-only`, `uppercase` и прочие `text-transform`), из `@feugene/unocss-mini-extra-rules`. По умолчанию `true`: компоненты этими утилитами пользуются, а без правил класс остаётся в разметке, и CSS к нему не появляется. |
...
| `pruneTokens`                           | Только `/node`. Обрезка неиспользуемых объявлений токенов в инлайнимом CSS. По умолчанию выключено (см. ниже). |

### `pruneTokens` (только node)

Убирает из инлайнимых `tokensCssUrl` и файлов тем объявления токенов, до
которых ничто не дотягивается. По умолчанию выключено. Полное описание — в
[Темах и токенах](./themes-and-tokens.md).

| Поле | Смысл |
|---|---|
| `mode` | `'off'` (по умолчанию) — эмиссия не меняется ни на байт; `'report'` — план считается и отдаётся, CSS прежний; `'on'` — план применяется. |
| `keep` | Имена токенов, сохраняемые безусловно. Строка допускает `*` в конце; `RegExp` сверяется с полным именем без `--`. |
| `keepPrefixes` | Сахар: `['gr-z-']` ≡ `keep: ['gr-z-*']`. |
| `appSources` | `{ dirs, extensions }` — исходники САМОГО приложения. Пресет видит компоненты провайдера и не видит вашу разметку; без этого `bg-[var(--brand)]` в `App.vue` для него не существует. |
| `scanLiterals` | Учитывать имена токенов, записанные строковым литералом в JS/TS/Vue. По умолчанию `true`. |

Внедрять через `'report'`: прогнать `granular prune`, прочитать список
удаляемого и только потом переводить в `'on'`.
