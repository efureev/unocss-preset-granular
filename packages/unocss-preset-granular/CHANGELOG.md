...
- **Диагностика `granular prune` шумела там, где всё сделано верно.** Токен,
  объявленный в `dynamicTokens` каким-то компонентом провайдера, попадал в
  находки, если этого компонента не было в селекции: имя лежит в общем чанке
  композабла, а владелец в сборку не вошёл — и токен удалён правильно.
  Измерено на `@feugene/granularity`: у приложения с одним `GrCard` находками
  становились оба токена шкалы слоёв, хотя объявлены они у восьми оверлейных
  компонентов. Теперь объявленный кем угодно токен находкой не считается —
  диагностика срабатывает только когда объявления нет ни у кого.
...
- **Обрезка неиспользуемых токенов — `pruneTokens`.** Пресет был селективен по
  компонентам, но фундамент провайдера инлайнился целиком: `tokensCssUrl` и
  файлы тем уезжали в CSS независимо от того, сколько компонентов выбрано. На
  реальном потребителе (`@feugene/granularity`) это 212 объявленных
  CSS-переменных там, где приложению нужны 17. Опция `pruneTokens.mode`
  (`off` по умолчанию | `report` | `on`) убирает объявления, до которых не
  дотягивается замыкание от реального потребления. Замерено на стенде
  репозитория: 117 объявленных токенов → 49, granular-слой 6 314 → 3 370 байт.

  Обрезка **текстовая**, а не пересборка из `{ selector, tokens }`: последняя
  потеряла бы блоки внутри at-rules, а именно там лежат `@supports`-фолбэки
  производных ролей. Не трогает `base.css` (там правила, а не объявления),
  компонентный CSS и структурные блоки тем.

- **`granular prune <options-file>`** — пятая команда CLI: что обрезка удалила
  бы, что сохранила и почему, с байтовой дельтой. Эмиссию не меняет.
  `--strict` выходит с кодом 1, если удалять есть что.

- **Канал потребления `source-literal`** в индексе токенов: имя токена,
  записанное строковым литералом (`'--gr-z-modal'`), без `var(`. Ловит три
  реальные записи, невидимые прежнему скану, — ключ инлайн-стиля, аргумент
  `setProperty`/`getPropertyValue` и имя, подставляемое в `var()` через
  переменную. Последнее не гипотеза: в `@feugene/granularity`
  `var(--gr-z-dropdown)` не встречается в исходниках ни разу, а токен
  потребляется.

- **`GranularComponentDescriptor.dynamicTokens`** — опциональное поле контракта
  (версия остаётся `1`): токены, к которым обращается КОМПОНЕНТ, собирая имя в
  рантайме. Обрезка держит их, пока компонент в сборке.

  Объявляется на компоненте, а не на провайдере, и это существенно.
  Провайдерский список держал бы токен в каждом приложении, включая те, где
  компонента нет: замерено на стенде — объявление на `XhOverlay` при селекции
  `XhPanel` (его граф включает оверлей) токен держит, при селекции `XhCard` —
  нет. Провайдерский держал бы в обоих. Плюс знание живёт рядом с кодом,
  который его порождает, а потребитель про рантайм-сборку внутри чужого
  компонента знать не может и не обязан.

- **Диагностика `granular prune`: шаблон, не совпавший ни с одним объявленным
  токеном.** `keep`, `keepPrefixes` и `dynamicTokens` с опечаткой либо
  протухшие после того, как компонент перестал собирать имя в рантайме. Само
  по себе ничего не ломает — потому и гниёт молча. Считаются все совпавшие
  шаблоны, а не первый: иначе второй, покрывающий тот же токен, числился бы
  мёртвым. Объявление НЕвыбранного компонента находкой не считается — оно
  просто не применилось к этой сборке.

- **Диагностика `granular prune`: удаляемый токен, чьё имя лежит литералом вне
  скан-директорий.** Ровно тот случай, где обрезка ломается молча: имя
  собирается в общем модуле, который бандлер вынес в чанк вне скана. Находкой
  считается имя в файле, который ТУТ ЖЕ собирает `var()` — без этого условия
  на реальной дизайн-системе срабатывает каждый удаляемый токен (измерено на
  `@feugene/granularity`: 195 из 195, все на TS-зеркале реестра токенов).
  С условием там остаются два настоящих — `--gr-z-dropdown` и `--gr-z-modal`
  в `chunks/overlayStack-*.js`.

- **`InlinedCssSource.kind`** (`tokens` | `base` | `theme`). Раньше роль файла
  восстанавливалась по отсутствию поля `theme`, то есть `base` и `tokens` были
  неразличимы; обрезке это различие нужно по существу.

- Публичные из `/node`: `granularTokenPrune`, `formatTokenPruneReport`,
  `planGranularTokenPrune`, `scanCssDeclarations`, `pruneCssDeclarations` и их
  типы.

### Changed

- **`@apply`/`@screen`/`theme()` раскрываются посекционно и ДО обрезки.**
  Раньше директивы раскрывались над уже собранным текстом, в
  `createGranularNodePreflight`. С обрезкой такой порядок стал неверным:
  `@apply` разворачивается в утилиты, которые вправе сослаться на токен через
  `var()`, а решение «этот токен никому не нужен» к тому моменту уже принято.
  `getGranularNodeCss` принимает необязательный второй аргумент — генератор.

### Fixed

- **`doctor` больше не считает находкой токен, видимый только строковым
  литералом.** Литерал — это чаще ПРИСВАИВАНИЕ (`{ '--gr-alert-bg': v }`,
  `setProperty`), а не чтение, и объявления от granular оно не требует. Без
  этой оговорки новый канал завалил бы находками `token-undefined` каждый
  пакет с покомпонентными токенами и уронил бы `doctor --strict` в чужом CI
  в день своего появления.


## [0.15.0] - 2026-08-28

Правки по фидбеку потребителя (`@feugene/granularity`, 78 компонентов)
