...
export interface GranularComponentDescriptor<Name extends string = string> {
   ...
   *
   * Критерий один: **собранный код компонента импортирует директорию другого
   * granular-компонента**. Практически это «мой шаблон или скрипт рендерит
   * чужой компонент» — не только шаблон: импорт из `<script setup>`,
   * композабла, render-функции или динамический `import()` считается ровно
   * так же.
   *
   * Забытое ребро означает, что у потребителя, выбравшего только этот
   * компонент, вложенный приедет без своих классов и без своего CSS: пресет
   * сканирует `components/<Name>/` и подмешивает `safelist` только тем, кто
   * попал в селекцию. Обратное тоже верно — импорт константы, типа или
   * утилиты из чужой директории зависимостью НЕ является: объявив его, вы
   * притащите всем своим потребителям весь CSS и safelist донора.
   *
   * Расхождение объявленного графа с тем, что реально отгружено в `dist`,
   * показывает `granular doctor` (диагностика `undeclared-dependency`).
   *
   * Все зависимости резолвятся через ЕДИНЫЙ РЕЕСТР компонентов, собранный со
   * всех зарегистрированных в `providers` пакетов. Ядро пресета рекурсивно
   * обойдёт граф `dependencies` и автоматически соберёт safelist/cssFiles
   * всех транзитивных зависимостей.
   */
...
/**
 * Блок строк пакета: адреса, по которым потребитель импортирует лоадеры локалей.
 *
 * Здесь только **адреса и состав**. Лоадеры-функции сюда класть нельзя: модуль
 * провайдера исполняется в конфиге потребителя, и функция утащила бы туда весь
 * locale-JSON, а резолвилась бы относительно провайдера, а не бандла приложения.
 * Пресет отдаёт адреса — импорты пишет потребитель.
 *
 * Имени блока здесь нет намеренно. Оно уже лежит внутри самой коллекции вторым
 * уровнем ключа (`{ en: { gr: () => import(...) } }`) и выводится из данных,
 * поэтому поле в контракте стало бы вторым источником правды об одной величине
 * и однажды разошлось бы с реальностью. Цена: столкновение блоков двух пакетов
 * не видно на сборке. Приемлемо — `fint-i18n` считает совпадение блоков штатным
 * мерджем лоадеров, а не ошибкой.
 */
export interface GranularI18nContribution {
  /**
   * Теги BCP 47, которые пакет объявляет: `en`, `ru`, `pt-BR`. Каждый из них —
   * верхний ключ своей `LocaleLoaderCollection` в `entry`.
   *
   * Нужны потребителю, чтобы отсечь неиспользуемые языки на сборке: без списка
   * остаётся только агрегат `allEntry`, который тянет всё.
   *
   * Именно ТЕГИ, а не имена импортов. Совпадать они обязаны не всегда: `pt-BR`
   * идентификатором быть не может, и пакет экспортирует его как `ptBR`. Имя
   * выводится из тега конвенцией (см. `exportNames`), а покрытие запрошенной
   * локали считается согласованием, а не `includes`: `ru-RU` обслуживается
   * лоадерами `ru`.
   */
  locales: readonly string[]
  /**
   * Имя именованного экспорта для тега — только там, где оно отличается от
   * самого тега.
   *
   * Разреженное переопределение поверх конвенции `fint-i18n` (дефис убирается,
   * следующий сегмент с заглавной: `pt-BR` → `ptBR`), а не второй список
   * локалей: ключи обязаны быть подмножеством `locales`. Обычный случай
   * `['en', 'ru']` этого поля не касается.
   */
  exportNames?: Readonly<Record<string, string>>
  /**
   * Подпуть с именованными per-locale экспортами. По умолчанию `${id}/i18n` —
   * конвенция `fint-i18n`, которой следуют все пакеты экосистемы.
   *
   * Пакеты экосистемы реэкспортируют оттуда же константу блока и адаптер,
   * поэтому импортировать нужно именованно. Что бы там ни лежало помимо
   * локалей, оно обязано быть side-effect-free: барель тянет каждый, кто пишет
   * `import { en }`, и рантайм-слой в нём уехал бы всем.
   */
  entry?: string
  /**
   * Подпуть-агрегат. По умолчанию `${entry}/all`.
   *
   * Форма отличается от `entry`: там именованные коллекции, здесь
   * **default-экспорт массива** коллекций. Предназначен для демо, e2e и тулинга —
   * в production-бандле равносилен отказу от tree-shaking языков.
   */
  allEntry?: string
}
...
export interface GranularProvider {
  ...
  /**
   * Блок строк пакета. Отсутствие поля и есть «у пакета нет встроенных строк» —
   * отдельного флага для этого не нужно.
   */
...
export interface DefineGranularComponentOptions<Name extends string = string> {
   ...
   * считается). Импорт константы или типа из чужой директории зависимостью
   * не является. Подробнее — {@link GranularComponentDescriptor.dependencies}.
   */
  dependencies?: readonly GranularComponentDependency[]
  safelist?: readonly string[]
  cssFiles?: readonly string[]
